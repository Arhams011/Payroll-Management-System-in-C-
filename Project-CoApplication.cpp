...
void saverecords()
{
    if (num == 0)
    {
        system("del Records.xls");
    }
    else
    {
        FILE *fp;
        fp = fopen("Records.xls", "w");

        // Print column headers
        fprintf(fp, "Name\tCode\tDesignation\tExperience\tAge\tSalary\tAnyLoan\tHRA\tPF\tTax\tMealAllowance\tTransportAllowance\tMedicalAllowance\tLoanBalance\tLoanDebit\tGrossPay\tWorkingHours\tDrivingAllowance\n");
        ...
        for (int i = 0; i < num; i++)
        {
            fprintf(fp, "%s\t%d\t%s\t%d\t%d\t%d\t%c\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n",
                    ...
                    emp[i].AnyLoan, emp[i].HRA, emp[i].PF, emp[i].tax, emp[i].MealAllowance, emp[i].TransportAllowance,
                    emp[i].MedicalAllowance, emp[i].LoanBalance, emp[i].LoanDebit, emp[i].grosspay, emp[i].workingHours,
                    emp[i].DA);
        }
...
void login()
{
    ...
    Password[i] = '\0';
    if(strcmp(UserName,"admin")==0 && strcmp(Password,"arham")==0)
    {
        system("cls");
        loginFrame1();
        gotoXY(27,10);
...
void intro()
{
    gotoXY(27,4); printf("\e[1mPAYROLL MANAGEMENT SYSTEM\e[0m");
    gotoXY(20,8); printf("");
    gotoXY(20,11); printf("");
    gotoXY(20,13); printf("");
    gotoXY(20,15); printf("");
...
void  list()
{
    ...
    borderNoDelay();
    gotoXY(20,4);
    printf("       \e[1m******** List of the Employees ********\e[0m");
    gotoXY(6,6);
    ...
    gotoXY(8,22);
    cout<<"\e[1mPress B to go back\e[0m";
    getch();
}
...
void menu()
{
    ...
    gotoXY(16,4);
    printf("\e[1m*****  Payroll Management System 1.0 ***** \e[0m");
    gotoXY(12,6);
...
void displayPayslip()
{
    ...
    for(i=0;i<=num-1;i++)
    {
        if(emp[i].code==code)
        {
            ...
            gotoXY(40,22);
            cout<<"\e[1mTotal Gross Pay : \e[0m"<<emp[i].grosspay;
        }
    }
    gotoXY(8,22);
    cout<<"\e[1mPress B to go back\e[0m";
